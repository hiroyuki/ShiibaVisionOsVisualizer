...
└── Documents/
    └── ShimonjuWoMotion/  ← ダンス名ごとにフォルダを作成
...
- PLYと同じ `ShimonjuWoMotion/` フォルダに音声ファイルを1つ置くと、アニメーション再生と同時に再生される
...
```bash
mkdir -p ~/Library/Mobile\ Documents/iCloud~jp~p4n~ShiibaVisionOsVisualizer/Documents/ShimonjuWoMotion
```
...
cp -r /path/to/ply_files/ \
  ~/Library/Mobile\ Documents/iCloud~jp~p4n~ShiibaVisionOsVisualizer/Documents/ShimonjuWoMotion/
```
